...
import com.wipro.its.listener.SubmitRatingListener;
...
public class SubmitRating extends JFrame
{
	public JLabel label,l1,l2,l3,l4;
	public JComboBox<String> t1,t2,t3;
	public JButton b1;
	public JButton b2;
	SubmitRatingListener rate=new SubmitRatingListener(this);
	...
	public SubmitRating()
	{
		...
		l4.setBounds(100,210,200,20);
		 ...
		 t1.setBounds(150,80,140,20);
			try
			{
				pst = con.prepareStatement("SELECT DISTINCT INTERVIEWID FROM ITS_TBL_INTERVIEW_SCHEDULE");
				rs = pst.executeQuery();
				while(rs.next())
				{
			...
			try
			{
				pst = con.prepareStatement("SELECT TECHID FROM ITS_TBL_INTERVIEW_SCHEDULE");
				rs = pst.executeQuery();
				while(rs.next())
				{
