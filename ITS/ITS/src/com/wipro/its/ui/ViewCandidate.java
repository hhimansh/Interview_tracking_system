...
import com.wipro.its.listener.ViewCandidateListener;
...
public class ViewCandidate extends JFrame
{
	...
	ViewCandidateListener v=new ViewCandidateListener(this);
	ViewCandidateTbl vct = new ViewCandidateTbl(this);
	public ViewCandidate()
	{
		...
		try
		{
			pst = con.prepareStatement("SELECT DISTINCT techid FROM ITS_TBL_INTERVIEW_SCHEDULE");
			rs = pst.executeQuery();
			while(rs.next())
			{
	...
	{
		ViewCandidate vi=new ViewCandidate();
	}
}
